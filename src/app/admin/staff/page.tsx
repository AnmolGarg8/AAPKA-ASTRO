...
export default function StaffManagementPage() {
  const { isOwner, role, email: userEmail, isLoading } = useCurrentUserRole();
  ...
  if (!isLoading && !isOwner && role !== "ADMIN") {
    return (
      <div className="min-h-screen bg-[#FBF3E7] p-8 flex items-center justify-center">
        <div className="max-w-md w-full bg-[#FFFDF9] border border-[#E8D8C3] rounded-3xl p-8 text-center space-y-4 shadow-sm">
          <div className="w-12 h-12 rounded-2xl bg-[#7B2D26]/10 text-[#7B2D26] mx-auto flex items-center justify-center">
            <Lock className="w-6 h-6" />
          </div>
          <h2 className="font-temple text-xl font-bold text-[#7B2D26]">Owner Only Access</h2>
          <p className="text-xs text-[#6E5545]">
            Per-section staff management is restricted to the platform Owner account (
            <code className="text-[#7B2D26] bg-[#FBF3E7] px-1 py-0.5 rounded font-mono">
              {process.env.NEXT_PUBLIC_OWNER_EMAIL || "anmol@aapkaastro.com"}
            </code>
            ).
          </p>
          <Link
            href="/dashboard"
            className="inline-block rounded-xl bg-[#7B2D26] text-white px-5 py-2 text-xs font-bold hover:bg-[#63231E]"
          >
            Return to Dashboard
          </Link>
        </div>
